...
import com.github.alexsniffin.packerman.impl.Packing.Packing
...
class GroupSpec extends FlatSpec with GivenWhenThen {
  ...
  "apply" should "return a group monad" in {
    Given("a pack object")
    val pack = Pack.create[TestClass](Some(Seq(TestClass("Test", 0))))
  ...
  "groupBy" should "return a packing instance" in {
    ...
    val group = Group(Pack.create[TestClass](Some(Seq(TestClass("Test", 0)))))
  ...
  "groupBy" should "return a packing instance that copied the pack object and added a fn" in {
    ...
    val pack = Pack.create(Some(Seq(TestClass("Test", 0))))
